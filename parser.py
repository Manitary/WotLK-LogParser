...
from encounters import creature_encounter, encounter_creature
from pet_recognition import permanent_pet_spells, pet_summon_pet, temporary_pet_spells
...
class parse:
    ...
    def generateFileName(self):
        try:
            with open(self.sourceFile, "r") as f:
                timestamp = re.search(
                    r"^(\d{1,2})\/(\d{1,2}) (\d{1,2}):(\d{1,2}):(\d{1,2})\.\d{1,3}",
                    f.readline(),
                )
    ...
    def populateDB(self):
            ...
            for line_num, line in enumerate(f.readlines()):
                try:
                    ...
                    if args[1].startswith("SP"):  # SPELL_
                        ...
                        query.bindValue(
                            ":spellName", args[9][1:-1] if args[9] != "nil" else None
                        )
                        query.bindValue(":spellSchool", int(args[10], 16))
                        i += 3
                        if (
                            suffix[0] == "P" or suffix[0] == "B"
                        ):  # PERIODIC_ | BUILDING_
                            suffix = suffix[9:]
                        ...
                        query.bindValue(
                            ":spellName", args[8][1:-1] if args[8] != "nil" else None
                        )
                        query.bindValue(":itemID", int(args[9]))
                        query.bindValue(
                            ":itemName", args[10][1:-1] if args[10] != "nil" else None
                        )
                    # parse suffix
                    if suffix:
                        if suffix.startswith("DA"):  # DAMAGE
                            ...
                            query.bindValue(
                                ":critical",
                                0 if args[i + 6] == "nil" else int(args[i + 6]),
                            )
                            query.bindValue(
                                ":glancing",
                                0 if args[i + 7] == "nil" else int(args[i + 7]),
                            )
                            query.bindValue(
                                ":crushing",
                                0 if args[i + 8] == "nil" else int(args[i + 8]),
                            )
                            ...
                            query.bindValue(
                                ":critical",
                                0 if args[i + 3] == "nil" else int(args[i + 3]),
                            )
                            ...
                            if suffix[-1] == "E":  # APPLIED_DOSE | REMOVED_DOSE
                                ...
                                query.bindValue(
                                    ":extraSpellName",
                                    args[i + 1][1:-1] if args[i + 1] != "nil" else None,
                                )
        ...
        QSqlQuery(
            "UPDATE events SET spellSchool = 32 WHERE sourceName = 'Shadowfiend' AND eventName LIKE 'SWING%'"
        ).exec()
        QSqlQuery(
            "UPDATE events SET spellSchool = 4 WHERE sourceName = 'Greater Fire Elemental' AND eventName LIKE 'SWING%'"
        ).exec()
        print("table created")
    ...
    def populateActors(self):
        ...
        players = QSqlQuery()
        players.prepare(
            "INSERT INTO actors (unitGUID, unitName, isPlayer, encounterTime) SELECT sourceGUID, sourceName, 1, :timeStart FROM events WHERE sourceGUID LIKE '0x__0%' AND (sourceFlags LIKE '%511' OR sourceFlags LIKE '%512' OR sourceFlags LIKE '%514') AND timestamp >= :timeStart AND timestamp <= :timeEnd GROUP BY sourceGUID"
        )
        pets = QSqlQuery()
        pets.prepare(
            "INSERT INTO actors (unitGUID, unitName, isPet, encounterTime) SELECT sourceGUID, sourceName, 1, :timeStart FROM events WHERE (sourceGUID LIKE '0x__4%' OR (sourceGUID LIKE '0x__3%' AND (sourceFlags LIKE '%1111' OR sourceFlags LIKE '%1112' OR sourceFlags LIKE '%1114'))) AND timestamp >= :timeStart AND timestamp <= :timeEnd GROUP BY sourceGUID"
        )
        npcs1, npcs2 = QSqlQuery(), QSqlQuery()
        npcs1.prepare(
            "SELECT sourceGUID, sourceName FROM events WHERE sourceGUID LIKE '0x__3%' AND sourceFlags LIKE '%a48' AND timestamp >= :timeStart AND timestamp <= :timeEnd GROUP BY sourceGUID"
        )
        npcs2.prepare(
            "SELECT targetGUID, targetName FROM events WHERE targetGUID LIKE '0x__3%' AND targetFlags LIKE '%a48' AND timestamp >= :timeStart AND timestamp <= :timeEnd GROUP BY targetGUID"
        )
        while encounter.next():
            ...
            players.exec()
            query.prepare(
                "SELECT unitGUID, unitName FROM actors WHERE isPlayer = 1 AND encounterTime = :timeStart"
            )
            ...
            pets.exec()
            query.prepare(
                "SELECT unitGUID, unitName FROM actors WHERE isPet = 1 AND encounterTime = :timeStart"
            )
            query.bindValue(":timeStart", t0)
            query.exec()
            pet_list = {}
            ...
            add_npc = QSqlQuery()
            add_npc.prepare(
                "INSERT INTO actors (unitGUID, unitName, isNPC, encounterTime) VALUES (:unitGUID, :unitName, 1, :timeStart) ON CONFLICT DO NOTHING"
            )
            add_npc.bindValue(":timeStart", t0)
            pet_npc = QSqlQuery()
            pet_npc.prepare(
                "UPDATE actors SET isNPC = 1 WHERE unitGUID = :unitGUID AND encounterTime = :timeStart"
            )
    ...
    def populateEncounters(self):
        ...
        getTime = QSqlQuery()
        getTime.prepare(
            "SELECT DISTINCT timestamp FROM events WHERE (sourceName = :unitName OR targetName = :unitName) AND (eventName LIKE '%DAMAGE' OR eventName = 'UNIT_DIED')"
        )
        insertTime = QSqlQuery()
        insertTime.prepare(
            "INSERT INTO encounters (unitGUID, enemy, timeStart, timeEnd, isKill) VALUES (:unitGUID, :enemy, :timeStart, :timeEnd, :isKill)"
        )
        ...
        for encounter in encounter_creature:
            ...
            encounter_GUID = ""
            query.prepare(
                "SELECT DISTINCT CASE WHEN sourceName = :unitName THEN sourceGUID ELSE targetGUID END FROM events WHERE sourceName = :unitName OR targetName = :unitName"
            )
    ...
    def assignPets(self):
        ...
        getOwner = QSqlQuery()
        getOwner.prepare(
            "INSERT INTO pets (petGUID, petName, ownerGUID, ownerName) SELECT CASE WHEN s.isPlayer = 1 THEN e.targetGUID ELSE e.sourceGUID END , CASE WHEN s.isPlayer = 1 THEN e.targetName ELSE e.sourceName END, CASE WHEN s.isPlayer = 1 THEN e.sourceGUID ELSE e.targetGUID END, CASE WHEN s.isPlayer = 1 THEN e.sourceName ELSE e.targetName END FROM events e JOIN actors s ON e.sourceGUID = s.unitGUID JOIN actors t ON e.targetGUID = t.unitGUID WHERE e.eventName = :eventName AND e.spellID = :spellID AND ((s.isPet = 1 AND s.isNPC IS NULL AND t.isPlayer = 1) OR (s.isPlayer = 1 AND t.isPet = 1 AND t.isNPC IS NULL)) GROUP BY e.sourceGUID, e.targetGUID ON CONFLICT DO NOTHING"
        )
        ...
        # Temporary pets
        getOwner.prepare(
            "INSERT INTO pets (petGUID, petName, ownerGUID, ownerName) SELECT e.targetGUID, e.targetName, e.sourceGUID, e.sourceName FROM events e JOIN actors s ON e.sourceGUID = s.unitGUID JOIN actors t ON e.targetGUID = t.unitGUID WHERE e.eventName = 'SPELL_SUMMON' AND e.spellID = :spellID AND s.isPlayer = 1 AND t.isPet = 1 AND t.isNPC IS NULL GROUP BY e.sourceGUID, e.targetGUID ON CONFLICT DO NOTHING"
        )
        ...
        getOwner.prepare(
            "INSERT INTO pets (petGUID, petName, ownerGUID, ownerName) SELECT e.targetGUID, e.targetName, p.ownerGUID, p.ownerName FROM events e JOIN pets p ON e.sourceGUID = p.petGUID WHERE e.eventName = 'SPELL_SUMMON' AND e.spellID = :spellID"
        )
        ...
        h = QSqlQuery()
        h.prepare(
            "SELECT e.timestamp, sourceGUID, sourceName, spec FROM events e JOIN specs s ON e.sourceGUID = s.unitGUID WHERE eventName = 'SPELL_CREATE' AND spellID = 34600 AND e.timestamp >= :timeStart AND e.timestamp <= :timeEnd AND s.timestamp = :timeStart ORDER BY id"
        )
        p = QSqlQuery()
        p.prepare(
            "INSERT INTO pets (petGUID, petName, ownerGUID, ownerName) SELECT targetGUID, targetName, :ownerGUID, :ownerName FROM events WHERE eventName = 'SPELL_SUMMON' AND spellID = 57879 AND timestamp >= :timeCast AND timestamp <= :timeExpire ORDER BY id LIMIT :limit ON CONFLICT DO NOTHING"
        )
        t.exec()
        while t.next():
            ...
            while h.next():
                ...
                p.bindValue(
                    ":timeExpire",
                    formatTimestamp(timeparse(s0) + datetime.timedelta(seconds=30))[
                        :-3
                    ],
                )
                if spec == "c3-3":
    ...
    def assignSpecs(self):
        ...
        getSpec = QSqlQuery()
        getSpec.prepare(
            "INSERT INTO specs (unitGUID, timestamp, unitName, spec) VALUES (:unitGUID, :timestamp, :unitName, :spec)"
        )
        ...
        getPlayerSpells.prepare(
            "SELECT timestamp, sourceGUID, spellID, sourceName, spellName FROM events JOIN actors ON events.sourceGUID = actors.unitGUID WHERE timestamp >= :timeStart AND timestamp <= :timeEnd AND (isPlayer = 1 or isPet = 1) AND spellID IS NOT NULL GROUP BY sourceGUID, spellID"
        )
        ...
        while getEncounters.next():
            ...
            while getPlayerSpells.next():
                ...
                if spellID in class_recognition.spell_spec:
                    ...
                    getSpec.bindValue(
                        ":spec",
                        f"{class_recognition.spell_spec[spellID]['class']}-{class_recognition.spell_spec[spellID]['spec']}",
                    )
                    getSpec.exec()
    ...
    def populateAuras(self):
        ...
        auras = QSqlQuery()
        auras.exec(
            "SELECT DISTINCT sourceGUID, sourceName, targetGUID, targetName, spellID, spellName, auraType, spellSchool FROM events WHERE eventName LIKE 'SPELL_AURA_%'"
        )
        find = QSqlQuery()
        find.prepare(
            "SELECT eventName, timestamp FROM events WHERE sourceGUID = :sourceGUID AND targetGUID = :targetGUID AND spellID = :spellID AND auraType = :auraType AND eventName LIKE 'SPELL_AURA_%' ORDER BY timestamp"
        )
        duration = QSqlQuery()
        duration.prepare(
            "SELECT duration FROM spell_db.spell_data WHERE spellID = :spellID"
        )
        new_aura = QSqlQuery()
        new_aura.prepare(
            "INSERT INTO auras (spellName, spellID, spellSchool, sourceGUID, sourceName, targetGUID, targetName, auraType, timeStart, timeEnd, eventType) VALUES (:spellName, :spellID, :spellSchool, :sourceGUID, :sourceName, :targetGUID, :targetName, :auraType, :timeStart, :timeEnd, :eventType)"
        )
        try:
            while auras.next():
                (
                ...
                print(
                    f"Current batch: {spellName} ({spellID}) - {sourceGUID} -> {targetGUID} - ({spellSchool}, {auraType})"
                )
                ...
                while find.next():
                    ...
                    elif find.value(0).startswith("SPELL_AURA_REFRESH") or find.value(
                        0
                    ).endswith("DOSE"):
                ...
                while applied:
                    ...
                    while search:
                        candidates = (
                            min(
                                (timeparse(x) for x in removed if x >= start),
                                default=None,
                            ),
                            min(
                                (timeparse(x) for x in refresh if x >= start),
                                default=None,
                            ),
                            (
                                timeparse(start)
                                + datetime.timedelta(milliseconds=auraDuration)
                            )
                            if auraDuration > 0
                            else None,
                        )
                ...
                while refresh:
                    ...
                    candidates = (
                        max(
                            (timeparse(x[1]) for x in data if x[1] and x[1] <= start),
                            default=None,
                        ),
                        (
                            timeparse(start)
                ...
                while removed:
                    ...
                    candidates = (
                        max(
                            (timeparse(x[1]) for x in data if x[1] and x[1] <= end),
                            default=None,
                        ),
                        (timeparse(end) - datetime.timedelta(milliseconds=auraDuration))
                        if auraDuration > 0
                        else None,
                    )
                    candidate = max((x for x in candidates if x), default=None)
    ...
    def assignAbsorbs(self):
        ...
        QSqlQuery(
            "UPDATE events SET absorbedByUnitName = 'Unknown' WHERE absorbed > 0 AND absorbedByUnitName IS NULL"
        ).exec()
